...
if os.path.exists(labelsFile):
    labels=pd.read_csv(labelsFile, skipinitialspace=True)
    slice=D.loc[starting:ending, labels.columns.get_values()]
else:
    slice=D.loc[starting:ending, :]
...
import umap
if transforming:
    embedding = trans.transform(slice)
else:
    ...
    if supervising:
        umap = reducer.fit(slice, y=target_slice)
        embedding = umap.embedding_
    else:
        umap = reducer.fit(slice)
        embedding = umap.embedding_
